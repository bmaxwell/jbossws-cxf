...
```
=======
>>>>>>> 9568ca632 ([JBWS-4444] Server throws IllegalStateException when call to SOAPHandler's getHeaders/handleFault/close methods is performed and their code include the CDI bean invocation)
...
=== Throtlling
The throttling feature allows server to limit the number of request when the server is busy or
...
throshold for endpoint metric value like requestCount, faultCount etc.
